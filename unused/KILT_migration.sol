...
contract KILTMigration is Ownable, Pausable, ReentrancyGuard {
    ...
    // Set the newToken address (owner only, one-time)
    ...
    // Migrate oldKILT to newKILT, burning old tokens safely
    function migrate(uint256 amount) external whenNotPaused nonReentrant {
        ...
        require(newToken.balanceOf(address(this)) >= newTokenAmount, "Insufficient new token balance");
        require(oldToken.allowance(msg.sender, address(this)) >= amount, "Approve contract to spend old tokens");

        // Burn old tokens first (external call to oldToken)
        require(oldToken.transferFrom(msg.sender, BURN_ADDRESS, amount), "Transfer to burn address failed");
        // Then transfer new tokens (external call to newToken)
        require(newToken.transfer(msg.sender, newTokenAmount), "New token transfer failed");
    }
